...
// .wrangler/tmp/bundle-cGIdKG/middleware-loader.entry.ts
init_modules_watch_stub();

// .wrangler/tmp/bundle-cGIdKG/middleware-insertion-facade.js
init_modules_watch_stub();

// src/index.js
init_modules_watch_stub();
var import_web_push = __toESM(require_src2(), 1);
var appOrigin = "https://saharatunsaeng-lang.github.io";
var checkinIntervalMs = 30 * 60 * 1e3;
var categoryCodes = {
  bd: "1 BD",
  sp: "2 SP",
  mm: "3 MM",
...
var TimeBlockSignal = class {
  static {
    __name(this, "TimeBlockSignal");
  }
  ...
  async scheduleNextCheckin(active) {
    ...
    const nextAt = Math.max(Date.now() + 1e3, startedAt + nextCount * checkinIntervalMs);
  ...
  async sendCheckin(active, reminderCount) {
    ...
    const payload = JSON.stringify({
      title: `${categoryCodes[active.categoryId] || active.categoryId} \xB7 ${reminderCount * 30} minutes`,
...
__name(isInstallationId, "isInstallationId");
function isActiveBlock(value) {
  return value && typeof value.id === "string" && typeof value.categoryId === "string" && typeof value.start === "string" && Number.isFinite(new Date(value.start).getTime());
}
__name(isActiveBlock, "isActiveBlock");
function isSubscription(value) {
  return value && typeof value.endpoint === "string" && value.endpoint.startsWith("https://") && typeof value.keys?.p256dh === "string" && typeof value.keys?.auth === "string";
}
...
function corsHeaders(origin) {
  return origin === appOrigin ? { "access-control-allow-origin": appOrigin, "access-control-allow-methods": "GET, POST, OPTIONS", "access-control-allow-headers": "content-type" } : {};
}
